...
function construccionCasas(bolsas) {
  // Tu código aquí:
    return function(casas) {
      ...
      if (bolsas < 10 || bolsas % 10 !== 0) {
        return "No se puede construir casas con esa cantidad de bolsas";
      }
      const casasPosibles = Math.floor(bolsas / 10);
      if (casas > casasPosibles) {
        return `Solo puedes construir esta cantidad de casas: ${casasPosibles}`;
      }
      return casas === casasPosibles;
    };
  }
  const casas = construccionCasas(100);
console.log(casas(10)); // Output: true
console.log(casas(0)); // Output: "Por favor ingresar cuantas casas quieres construir"
console.log(casas(2)); // Output: "Solo puedes construir esta cantidad de casas: 1"
console.log(casas(11)); // Output: true
console.log(casas(9)); // Output: "No se puede construir casas con esa cantidad de bolsas"
